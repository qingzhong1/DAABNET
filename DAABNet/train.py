...
def val(args, val_loader, model):
    ...
    for i, (input, label, size, name) in enumerate(val_loader):
        ...
        #output,x_guide = model(input_var)
...
def train(args, train_loader, model, criterion, optimizer, epoch):
    ...
    for iteration, batch in enumerate(train_loader, 0):
        ...
        #args.max_iter = (args.max_epochs+500) * args.per_iter
        args.cur_iter = epoch * args.per_iter + iteration
        #args.cur_iter = (epoch+500) * args.per_iter + iteration
        ...
        #output,x_guide = model(images)
        output = model(images)
        #print('#############################',output)
        #print('#############################',labels.shape)
        #print('#############################',labels)
        output_color = output.cpu().data[0].numpy()
        gt = np.asarray(labels[0].cpu().numpy(), dtype=np.uint8)
        output_color = output_color.transpose(1, 2, 0)
        output_color = np.asarray(np.argmax(output_color, axis=2), dtype=np.uint8)
        
        #print(name[0][0])
        args.save=True
        if args.save:
            save_predict(output_color, gt, name[0], args.dataset, 'result_train',
                         output_grey=False, output_color=True, gt_color=True)
        loss_1 = criterion(output, labels)
        #loss_2 =  criterion(x_guide, labels)
        #boundery_bce_loss8,  boundery_dice_loss8=boundary_loss_func(x_guide,labels)
        #loss=loss_1+boundery_bce_loss8+boundery_dice_loss8
