...
class DetailAggregateLoss(nn.Module):
    def __init__(self, *args, **kwargs):
        ...
        self.conv1 = torch.nn.Conv2d(128,1,(1,1)).cuda()

    def forward(self, boundary_logits, gtmasks):

        # boundary_logits = boundary_logits.unsqueeze(1)
        #boundary_logit=self.conv1(boundary_logits)
        ...
        #print(gtmasks.unsqueeze(1).shape)
        ...
        boudary_targets_pyramid = F.conv2d(boudary_targets_pyramids, self.fuse_kernel)   #在此处修改通道数
        ...
        if boundary_logits.shape[-1] != boundary_targets.shape[-1]:
            #boundary_logits=self.conv1(boundary_logits)
            boundary_logits = F.interpolate(
                boundary_logits, boundary_targets.shape[2:], mode='bilinear', align_corners=True)
            #boundary_logits=self.conv1(boundary_logits)
        
        #print(boundary_logits.shape,boudary_targets_pyramid.shape)
