...
def test(args, test_loader, model):
    ...
    #for i, (input, label) in enumerate(test_loader):
        ...
        output,x_guide = model(input_var)   #使用了细节损失
        ...
        #output = F.interpolate(output, size=label.shape[-2:], mode='bilinear', align_corners=True)
        ...
        #print(gt.shape)
        #print((label[keep] * n_classes).shape)
        
        #print(output[keep].shape)
        #hist += torch.bincount(torch.tensor(label[keep] * n_classes) + torch.tensor(output[keep]), minlength=n_classes**2).view(n_classes, n_classes)
        #print(output.shape)
    ...
    #print('****************',per_class_iu)
    #return  miou,ious.cpu().numpy().tolist(),
...
def test_model(args):
    ...
    #transform = get_val_transform([360,480])
    
    #root="dataset/camvid/"
    #dataset = CamVid(root, split="test", transform=transform)
    #testLoader = DataLoader(dataset, 1, num_workers=1, pin_memory=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint', type=str,
                        default="./checkpoint/cityscapes/DABNetbs4gpu1_train/model_1000.pth",
